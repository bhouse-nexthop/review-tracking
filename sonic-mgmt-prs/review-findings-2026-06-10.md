...
| PR | Type | Cmplx | Matches desc? | Existing approvals | Conflict | Dup | Headline flag for reviewer |
|----|------|-------|---------------|--------------------|----------|-----|----------------------------|
| [#23930](https://github.com/sonic-net/sonic-mgmt/pull/23930) | Feature (hwsku reg) | Low | Yes | none | Low | none | Confirm TH5 chip classification; vmittal-msft asked for descriptive title + master PR (open) |
| [#24320](https://github.com/sonic-net/sonic-mgmt/pull/24320) | Mix (refactor+fixes) | Med-High | **Partial** | none (commented) | Low | none | Scope creep beyond description; abdosi says approach may be superseded; risky wholesale PORT replace |
| [#24437](https://github.com/sonic-net/sonic-mgmt/pull/24437) | Feature (diag) | Low | Yes | nhe-NV | Low | none | Clean |
| [#24493](https://github.com/sonic-net/sonic-mgmt/pull/24493) | Bug fix | Low | Yes | nhe-NV | Low | none | Clean; confirm auth coverage preserved elsewhere |
| [#24545](https://github.com/sonic-net/sonic-mgmt/pull/24545) | Bug fix | Med | Yes | StormLiangMS, ZhaohuiS | **Med** (24845 shared yaml) | none | VS recovery timing assumption; post-recovery interface assertion removed |
| [#24597](https://github.com/sonic-net/sonic-mgmt/pull/24597) | Bug fix | Med | Yes | nhe-NV | Low | none | Confirm `sonic_deploy_202505.j2` template name + IPv6 asic set |
| [#24787](https://github.com/sonic-net/sonic-mgmt/pull/24787) | New test suite | Low-Med | Yes | none | Low | none | Destructive (reload+cold reboot); no `Fixes #` link; no human review yet |
| [#24829](https://github.com/sonic-net/sonic-mgmt/pull/24829) | Bug fix | Low | Yes | none | Low | none | No test added; confirm `ports` scope + ASIC-mode coverage |
| [#24845](https://github.com/sonic-net/sonic-mgmt/pull/24845) | New test suite | Med | Yes | none | Low (24545) | none | Live CodeQL regex warnings; `acl.json` action/value mismatch; slow/flaky-prone |
| [#24876](https://github.com/sonic-net/sonic-mgmt/pull/24876) | Bug fix (flake) | Low | Yes | ZhaohuiS, prhoskot | Low | none | Comment/code mismatch on stability gate (cosmetic) |
| [#24884](https://github.com/sonic-net/sonic-mgmt/pull/24884) | Test improvement | Low | Yes | none | Low | none | Redis `save` restore edge case; 1s→5s poll coarsens convergence resolution globally |
| [#24902](https://github.com/sonic-net/sonic-mgmt/pull/24902) | Bug fix | Low | Yes | none | Low | none | Changes shared `wait_until` semantics (fast-fail → retry); title typo |
| [#24927](https://github.com/sonic-net/sonic-mgmt/pull/24927) | Bug fix | Low | Yes | nhe-NV, yyynini | Low | none | Fixture swap changes DUT-enumeration for all topologies; unguarded VLAN index |
| [#24930](https://github.com/sonic-net/sonic-mgmt/pull/24930) | Test improvement | Low | Yes | none | Low | none | Author affiliation **unknown**; `/tmp/{file}` path assumption in cleanup |
| [#24975](https://github.com/sonic-net/sonic-mgmt/pull/24975) | Bug fix | Low | Yes | nhe-NV | Low | none | Fixed 3s pause is a timing band-aid (vs readiness wait); low risk |
| [#25000](https://github.com/sonic-net/sonic-mgmt/pull/25000) | Bug fix | Low | Yes | none | Low | none | Sibling `test_srv6_dataplane.py` arguably needs same fix, not included |
| [#25012](https://github.com/sonic-net/sonic-mgmt/pull/25012) | Bug fix | Med | Yes | liamkearney-msft | Low | none | Clean; minor wait_until timeout doc/code drift (50 vs 120) |
| [#25040](https://github.com/sonic-net/sonic-mgmt/pull/25040) | Bug fix (flake) | Med | Yes | shixizhang | Low | none (25094 same marker, diff file) | Confirm `get_bbr_default_state`/`safe_remove_aggregate` pre-exist |
| [#25094](https://github.com/sonic-net/sonic-mgmt/pull/25094) | Test improvement | Low | Yes | nhe-NV | Low | none | Author affiliation **unknown**; clean idiomatic change |
| [#25123](https://github.com/sonic-net/sonic-mgmt/pull/25123) | Bug fix | Med | Yes | none | Low | none | Intentional syslog-sampling sensitivity trade-off (sign-off needed); hardcoded platform string |
| [#25134](https://github.com/sonic-net/sonic-mgmt/pull/25134) | Bug fix (enable) | Low | Yes | none | Low | none | Re-enables a previously-skipped test — flake risk in gating |
...
- **Reviewer notes:** Correct, but `Juniper-QFX5241-64-OD` is added to `broadcom_th5_hwskus` (Tomahawk5) — a Juniper/SME should confirm the chip generation. Outstanding process items from vmittal-msft (descriptive title + companion master PR).

### PR #24320 — changes for port speed test enhancement
...
### PR #24437 — save pfcwd_timer_accuracy test result to file
...
### PR #24493 — Update the gNMI setup without authentication for liquid cooling case
...
### PR #24545 — Fix test_monitoring_critical_processes timeout by killing database container last
...
### PR #24597 — Update the fanout switch deploy step due to lack of /etc/sysctl.conf
...
### PR #24787 — [platform_tests][T2] Add test_sup_fan_recovery.py
...
### PR #24829 — Fix: add port name for acl interface parsing
...
### PR #24876 — lldp_syncd failure due to not enough converge time
...
### PR #24884 — [bgp scale] Reduce DUT-side observer load during route convergence tests
...
### PR #24902 — Handle pytest.fail.Exeption in wait_until
...
### PR #24927 — Fix test_link_local_ip failures in dualtor active-active topology
...
### PR #24930 — [vxlan] Improve vnet bgp subintf cleanup diagnostics
...
### PR #24975 — Fix the NTP polling step in deploy-mg playbook
...
### PR #25000 — Fix test_srv6_vlan_forwarding when no ipv6 mgmt for ptf docker
- **Author / affiliation:** ytzur1 / NVIDIA
- **Type:** Bug fix
...
### PR #25012 — Fixing PMON status test failures
...
### PR #25040 — [bgp/agg] Make BGP aggregate-address tests solid against per-DUT flakiness
...
### PR #25094 — Add disable_memory_utilization option for fwutil test cases
...
### PR #25123 — [TH6] test_po_cleanup fix for large number of lags(100s)
...
### PR #25134 — [conditional_mark]: Enable NTP IPv6-only management test on sonic-vpp
